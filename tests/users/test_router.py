...
def test_user_delete_failure(test_client, monkeypatch, token, user_id_in_path,
    ...
    # 토큰 검증 함수를 가짜로 교체
    monkeypatch.setattr('core.auth.get_token_payload', lambda token: None
    ...
    response = test_client.post("/users/delete", headers={"Authorization": f"Bearer {token}"},
...
def test_login_user_failure(test_client, monkeypatch, email, password, expected_status, expected_detail):
    ...
    monkeypatch.setattr('users.services.find_user_with_email', lambda db, email: None
                                                                if email == "nonexistent@example.com"
                                                                else User(email="valid@example.com",
