...
def update_user_info(db: Session, user: schema.UserEdit):
    ...
    qry = (
        update(model.User).where(model.User.user_id == user.user_id).values(username=user.username, password=password)
    )
    db.execute(qry)
    db.commit()
    return user
...
def generate_login_token(db: Session, user_info: schema.UserLogin):
    ...
    at = auth.generate_tokens(
        user_info.user_id, datetime.now(KST) + timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINS)
    )
